	...
	return (
		<button onClick={() => functionOnClick?.()}
			...
			<div className="flex justify-between w-full">
				...
				{topRightText &&
					<p className="text-gray-500">
						{typeof topRightText == 'string' ? topRightText : topRightText.icon + topRightText.text}
					</p>
				}
			</div>
			...
			<div className="absolute bottom-0 right-0 py-2 px-4 rounded-tl-xl border-t border-l
				border-gray-100
				cursor-pointer duration-500 bg-white"><p>{bottomRightCard?.text}
				<span className="text-xl">{bottomRightCard?.icon}</span></p></div>
		</button>
	);
};
